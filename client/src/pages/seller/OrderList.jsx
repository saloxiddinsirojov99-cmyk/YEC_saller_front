...
import { getOrders, getOrderById, updateOrder, deleteOrder } from '../../services/api';
...
export default function OrderList() {
  ...
  // Check if any order needs delivery confirmation (today >= delivery_date and status pending)
  useEffect(() => {
    if (orders.length > 0) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const updated = await updateOrder(editModal.id, updateData);
      ...
      <div className="order-list full-width">
        ...
        <div className="filter-section">
          ...
          <button
            ...
            onClick={() => { setFilter('pending'); handleSearch(); }}
          >
            Kutilmoqda ({orders.filter(o => o.status === 'pending').length})
          </button>
          <button
            className={`filter-btn ${filter === 'completed' ? 'active' : ''}`}
            onClick={() => { setFilter('completed'); handleSearch(); }}
          >
                ...
                <div className="edit-grid">
                  <div className="edit-column">
                    ...
                    <div className="form-group">
                      ...
                      <input type="text" value={editForm.customer_phone2 || ''} onChange={(e) => setEditForm({ ...editForm, customer_phone2: e.target.value })} />
                    </div>
                    <div className="form-group">
                      <label>Manzil</label>
                  ...
                  <div className="edit-column">
                    ...
                    <div className="form-group">
                      ...
                      <input type="number" value={editForm.paid_amount} onChange={(e) => setEditForm({ ...editForm, paid_amount: parseFloat(e.target.value) || 0 })} min="0" />
                    </div>
                    <div className="form-group">
                      <label>Izoh</label>
              ...
              return (
                <div key={order.id} className={`table-row ${isDeliveryDue ? 'delivery-due' : ''}`}>
                  ...
                  <div className="col-amount">
                    <span className="amount">{order.total_amount?.toLocaleString()} so'm</span>
                  </div>
